...
### Option#1. Use [Azure Custom Script Extension](https://docs.microsoft.com/en-us/azure/virtual-machines/extensions/custom-script-linux) to install blobfuse driver on every agent VM
 ...
 > Note: for AKS cluster, all VM resources are under a shadow resource group naming as `MC_{RESOUCE-GROUP-NAME}{CLUSTER-NAME}{REGION}`; check `sudo vi /var/lib/waagent/custom-script/download/1` if there is failure
```
az vm extension set \
